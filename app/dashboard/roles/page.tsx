...
export default function RoleManagementPage() {
  // Permission matrix state — tracks which permissions each role has
  const [permissionMatrix, setPermissionMatrix] = useState(defaultPermissions);

  // Selected role for the modal
  const [selectedRole, setSelectedRole] = useState<Role | null>(null);

  // Modal open states
  const [roleModalOpen, setRoleModalOpen] = useState(false);

  // Temp permissions state used inside the modal before saving
  const [tempPermissions, setTempPermissions] = useState<string[]>([]);

  // Open the role modal with the selected role's current permissions
  ...
  // Toggle a permission inside the modal
  ...
  // Save changes from the modal back to the permission matrix
  ...
  return (
    <div className="space-y-6">
        ...
        {initialRoles.map((role, index) => (
          ...
          >
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              {/* Manage Role button — also opens role modal */}
              <button
                onClick={() => handleOpenRoleModal(role)}
      ...
      >
        <h2 className="text-xl font-semibold text-gray-900 mb-6">Permission Matrix</h2>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">Permission</th>
                {initialRoles.map((role) => (
                  <th key={role.name} className="text-center py-3 px-4 text-sm font-medium text-gray-700">
                    {role.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {permissionsList.map((permission, index) => (
                ...
                >
                  <td className="py-4 px-4 text-sm text-gray-900">{permission.name}</td>
                  {initialRoles.map((role) => {
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-gray-900">User Assignment</h2>
      ...
      <Dialog.Root open={roleModalOpen} onOpenChange={setRoleModalOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto z-50">
            {selectedRole && (
              <div className="p-8">
                ...
                <div className="flex space-x-3">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 text-white rounded-lg hover:opacity-90 transition-opacity font-medium bg-gradient-to-r ${selectedRole.color}`}
                  >
                    Save Changes
                  </button>
                </div>
